...
Status: **complete** — Phase 1 landed the `BigInt` type; Phase 2 made the
combinatorial sequence verbs arbitrary precision; **Phase 3 (this PR)**
reimplements `Rational` on `BigInt`, so the whole exact symbolic tower is
arbitrary precision. Size: **L**. Written to DESIGN.md's contract standard:
normative sections fold into DESIGN.md as each phase lands.
